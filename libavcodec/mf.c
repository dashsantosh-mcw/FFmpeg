...
typedef struct MFContext {
    ...
    AVFrame *frame;
    int is_video, is_audio, is_enc, is_dec;
    GUID main_subtype;
    IMFTransform *mft;
...
static int mf_create(void *log, MFFunctions *f, IMFTransform **mft, const AVCodec *codec, int use_hw)
{
    ...
    *mft = NULL;
    ...
    if (is_dec) {
        ...
        if ((ret = ff_instantiate_mf(log, f, category, &reg, NULL, use_hw, mft)) < 0)
            return ret;
    } else {
        if (is_audio) {
            reg.guidMajorType = MFMediaType_Audio;
...
static int mf_init_codec(AVCodecContext *avctx)
{
    MFContext *c = avctx->priv_data;
    HRESULT hr;
    int ret;
    ...
    if (c->is_video && c->is_enc && c->opt_enc_hw)
        use_hw = 1;
    ...
    if (c->is_enc && 
        avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER && c->async_events &&
