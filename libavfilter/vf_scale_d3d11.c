...
static av_cold int d3d11scale_init(AVFilterContext *ctx) {
    // Simple initialization - all real work is done in config_props and filter_frame
    return 0;
}

static void release_d3d11_resources(D3D11ScaleContext *s) {
    // Safe release of D3D11 resources
    if (s->outputView) {
    ...
    // Note: device and context are referenced from hw_device_ctx, don't release directly
}
